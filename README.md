<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,18,24&height=200&section=header&text=Cengizhan%20OZBEK&fontSize=52&fontColor=ffffff&animation=fadeIn&fontAlignY=36&desc=Co-Owner%20%26%20Developer%20%40%20Zoria%20Studio%20•%20Fullstack%20JS%20%2F%20Game%20Dev&descSize=17&descAlignY=58" width="100%"/>

<a href="https://github.com/S7venz">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&duration=2800&pause=700&color=8B5CF6&center=true&vCenter=true&width=620&lines=Co-Owner+%26+Developer+%40+Zoria+Studio+(MMORPG);Fullstack+JS+%7C+React+%2F+Node+%2F+Express;Game+dev+with+C%2B%2B+%2F+Python+%2F+Lua;Machine+Learning+%26+GenAI+%7C+TensorFlow+%2F+RAG;MSc+Cybersecurity+%26+High+Availability+(RNCP+7)" alt="Typing SVG"/>
</a>

<br/>

<img src="https://komarev.com/ghpvc/?username=S7venz&label=Profile%20views&color=8B5CF6&style=flat-square" alt="profile views"/>
<a href="https://github.com/S7venz?tab=followers"><img src="https://img.shields.io/github/followers/S7venz?style=flat-square&color=8B5CF6&labelColor=12181f&logo=github" alt="followers"/></a>
<img src="https://img.shields.io/badge/Based%20in-Montpellier,%20FR-8B5CF6?style=flat-square&labelColor=12181f" alt="location"/>

</div>

---

### About me

- **Co-Owner & developer** at **Zoria Studio** — an MMORPG game studio: product vision, team management and technical coordination.
- Game development in **C++ / Python / Lua** (**MySQL 8** backend) and administration of a **Linux/FreeBSD** server (deployment, monitoring, hardening, backups).
- **Fullstack JS**: React / Node.js / Express, **REST APIs**, real-time with **Socket.IO**, relational databases, Docker & CI/CD.
- **Machine Learning & GenAI**: TensorFlow / Keras, **LLM / RAG**.
- Student at **L'IDEM** — MSc in IT Project Management, **Cybersecurity & High Availability** track (RNCP 7, starting 2026).

---

### Tech stack

<div align="center">

<img src="https://skillicons.dev/icons?i=js,ts,python,java,php,cpp,lua,react,nodejs,express,html,css,tensorflow,mysql,postgres,redis,docker,gradle,git,gitlab,linux,bash&perline=8" alt="tech stack"/>

</div>

---

### GitHub stats

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=S7venz&show_icons=true&count_private=true&hide_border=true&theme=tokyonight&title_color=8B5CF6&icon_color=8B5CF6" alt="github stats"/>
<img height="170" src="https://streak-stats.demolab.com?user=S7venz&hide_border=true&theme=tokyonight&ring=8B5CF6&fire=8B5CF6&currStreakLabel=8B5CF6" alt="streak"/>

<br/>

<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=S7venz&layout=compact&hide_border=true&langs_count=8&theme=tokyonight&title_color=8B5CF6" alt="top languages"/>

<br/><br/>

<img src="https://github-profile-trophy.vercel.app/?username=S7venz&theme=tokyonight&no-frame=true&no-bg=true&margin-w=4&row=1&column=7" alt="trophies"/>

<br/>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=S7venz&theme=tokyo-night&hide_border=true&area=true&color=8B5CF6&line=8B5CF6&point=ffffff" width="95%" alt="activity graph"/>

</div>

---

### Featured projects

- **[card7games.fr](https://github.com/S7venz/card7games)** — Three card games (UNO, 6 nimmt!, War) in solo/multiplayer with AI, React / Node.js / Socket.IO, persistent leaderboard.
- **[covoit-app](https://github.com/S7venz/covoit-app)** — Carpooling mobile app (React Native).
- **[animal-guesser-model](https://github.com/S7venz/animal-guesser-model)** — Animal species image classification with a CNN (TensorFlow / Keras, Jupyter).
- **[M2-ITEM-EDITOR](https://github.com/S7venz/M2-ITEM-EDITOR)** — Item editor for Metin2 (game dev tooling).
- **[life-hex-game](https://github.com/S7venz/life-hex-game)** — Multiplayer Game of Life on a hexagonal grid, adaptive AI, Node.js + Socket.IO, configurable rules.
- **[hook-cook](https://github.com/S7venz/hook-cook)** — E-commerce platform for a fishing tackle shop (store, permits, contests, admin). Grails 6/Groovy + React 19/Vite, PostgreSQL + Redis, JWT/BCrypt, Docker, security-focused (OWASP, GDPR).

---

<div align="center">

### Let's connect

<a href="https://www.linkedin.com/in/cengizhan-o-9150b823b/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="linkedin"/></a>
<a href="mailto:cengizhanozbek02@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="email"/></a>
<a href="https://github.com/S7venz"><img src="https://img.shields.io/badge/GitHub-12181f?style=for-the-badge&logo=github&logoColor=white" alt="github"/></a>

<br/><br/>


<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,18,24&height=120&section=footer" width="100%"/>

</div>